...
import { AutoComplete, AutoCompleteProps, Button, Divider, Dropdown, Flex, Input, MenuProps, message, Modal, Popover, Select, Space, Tag } from "antd";
...
export default function TagsModal(prop:any){
    ...
    const items: MenuProps['items'] = [
        {
          ...
          label: (
            <div onClick={()=>{
                selectTagsSort(newStore.language,"update_time","asc").then(res=>{
                    console.log(res);
                    if(res.code == 0){
        ...
        },
        {
          ...
          label: (
            <div onClick={()=>{
                selectTagsSort(newStore.language,"use_cnt","asc").then(res=>{
                    console.log(res);
                    if(res.code == 0){
                        let temp = Array.from(res.data,(item:any)=>{
                            return item.tag
    ...
    const handleRemoveTags = ()=>{
        ...
        cleanTags.forEach((tag)=>{
            removeTags(newStore.language,tag)
        })
    }
    // 返回上一层
