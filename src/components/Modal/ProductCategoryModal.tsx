...
export default function ProductCategoryModal(prop:any) {
    ...
    function buildTree(data, parentId = '0') {
        return data.filter(item => item.pid === parentId).map(item => ({
            ...item,
            value: item.id,
            ...
            >
                <Scoped>
                    ...
                    {/* <div className="column4">
                        <Input prefix={<SearchOutlined />} placeholder="搜索分类" onChange={handleSearchTags} />
                    </div>
                    <div>
                        {searchTags.length === 0?<div style={{fontSize:"12px",color:"#7A8499"}}>未找到对应分类</div>:<Flex gap="4px 0" wrap>
