...
function formatDateTime(date:any) {  
  ...
  const dateTimeStringWithoutTime = date.toLocaleString(init.locale, {  
    ...options,  
  ...
  // 拼接最终的字符串  
  return `${init.formatMessage({ id: 'shopTimeDisplay.shopTimeDisplay' })}：${isPM ? init.formatMessage({ id: 'shopTimeDisplay.afternoon' }) : init.formatMessage({ id: 'shopTimeDisplay.morning' })}${formattedHour}:${minutes} ${dateTimeStringWithoutTime.slice(dateTimeStringWithoutTime.lastIndexOf('年') + 1)}`;  
}  
