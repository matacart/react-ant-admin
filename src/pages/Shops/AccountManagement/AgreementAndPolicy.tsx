...
function AgreementAndPolicy() {
    ...
    // 注册时间
    const regTime = dayjs(Number(accountManagement.userInfo.reg_time)*1000).format("YYYY-MM-DD HH:mm:ss")
    // 操作时间
    const dataSharingTime = dayjs(Number(accountManagement.userInfo.data_sharing_time)*1000).format("YYYY-MM-DD HH:mm:ss")
    return (
        <Scoped>
            <Card>
                ...
                <Flex className="switch-box">
                    <Switch checked={accountManagement.userInfo.data_sharing == "1"?true:false} onChange={(checked)=>sharedState(checked)} />
                    {accountManagement.userInfo.data_sharing == "1" ? <div className="color-474F5E switch-text">已开启<span className="font-12 color-7A8499">（于 {accountManagement.userInfo.data_sharing_time == null?regTime:dataSharingTime} 手动操作开启）</span></div>:<div className="color-474F5E switch-text">已关闭<span className="font-12 color-7A8499">（于 {accountManagement.userInfo.data_sharing_time == null?regTime:dataSharingTime} 手动操作关闭）</span></div>}
                </Flex>
            </Card>
        </Scoped>
    )
