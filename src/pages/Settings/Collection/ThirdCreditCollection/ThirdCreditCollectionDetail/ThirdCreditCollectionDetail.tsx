...
function ThirdCreditCollectionDetail() {
    ...
    const fetch = (langId:string)=>{
        getAddonsConfigCreditCard(searchParams.get("id") || "",searchParams.get("addonsId") || "",langId).then(async res=>{
            if(res.data.addon_config){
                ...
                await collection.setOldThirdCreditCollection(res.data.addon_config)
                Object.entries(JSON.parse(res.data.addon_config.config)).forEach(([key, value]) => {
                    collection.setOldThirdCreditCollection({
                        ...collection.oldThirdCreditCollection,
                        ["config["+key+"]"]:value
                    })
                    setRawData({
                        ...collection.oldThirdCreditCollection,
    ...
    const submit = ()=>{
        setLoading(true)
        setAddonsConfigs(collection.oldThirdCreditCollection).then(res=>{
            message.success("已更新")
        }).catch(()=>{
        }).finally(()=>{
            setLoading(false)
                ...
                <div className="mc-layout">
                    <div className="mc-header">
                        ...
                        <div className="mc-header-right">
                            {/* 语言 */}
                            <LangSelect lang={collection.oldThirdCreditCollection.languages_id} setLang={setLang} />
                        </div>
                    </div>
                    <div className='mc-layout-main'>
                        <div className='mc-layout-content'>
                                ...
                                <div style={{margin:"20px 20px 0 20px"}}>
                                    <Form form={form} layout="vertical">
                                        {Object.entries(formItemList.config).map(([key, value])=>(
                                            <Form.Item key={key} label={<div className="font-w-600">{value.title}</div>}>
                                                {(()=>{
                                                    switch(value.type){
                                                        case "text":
                                                            return <Input value={(collection.oldThirdCreditCollection as Record<string, any>)["config["+key+"]"] || ""} placeholder={value.title} onChange={(e)=>{
                                                                collection.setOldThirdCreditCollection({
                                                                    ...collection.oldThirdCreditCollection,
                                                        ...
                                                        case "select":
                                                            ...
                                                            return <Select value={(collection.oldThirdCreditCollection as Record<string, any>)["config["+key+"]"] || null} placeholder={value.title} options={options} onChange={(value)=>{
                                                                collection.setOldThirdCreditCollection({
                                                                    ...collection.oldThirdCreditCollection,
                                                                    ["config["+key+"]"]:value
                                                                })
                                                        ...
                                                        case "radio":
                                                            return <Radio.Group
                                                                value={(collection.oldThirdCreditCollection as Record<string, any>)["config["+key+"]"] || "0"}
                                        ...
                                        <Form.Item label={<div className="font-w-600">自定义标题</div>}>
                                            <Input value={collection.oldThirdCreditCollection.title} onChange={(e)=>{
                                                collection.setOldThirdCreditCollection({
                                                    ...collection.oldThirdCreditCollection,
                                        ...
                                        <Form.Item label={<div className="font-w-600">自定义描述</div>}>
                                            <TextArea showCount maxLength={500} value={collection.oldThirdCreditCollection.description} rows={5} onChange={(e)=>{
                                                collection.setOldThirdCreditCollection({
                                                    ...collection.oldThirdCreditCollection,
                                        ...
                                        <Form.Item label={<div className="font-w-600">备注</div>}>
                                            <TextArea showCount maxLength={200} value={collection.oldThirdCreditCollection.remark} rows={2} onChange={(e)=>{
                                                collection.setOldThirdCreditCollection({
                                                    ...collection.oldThirdCreditCollection,
                                        ...
                                        <Form.Item label={<div className="font-w-600">排序</div>}>
                                            <Input value={collection.oldThirdCreditCollection.sort} onChange={(e)=>{
                                                collection.setOldThirdCreditCollection({
                                                    ...collection.oldThirdCreditCollection,
                                                    sort:e.target.value
                                                })
                            ...
                            <Flex justify="space-between">
                                {collection.oldThirdCreditCollection.status == 1?<DeleteModal tElement={
