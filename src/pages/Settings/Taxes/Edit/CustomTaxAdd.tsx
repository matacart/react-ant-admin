...
function CustomTaxAdd() {
    ...
    const submit = () => {
        form.validateFields().then((values) => {
            let taxesName;
            if(values.type == "2"){
                const category = categoryList.find((item:any)=>item.value == values.category);
                taxesName = category?.label || "";
            }else{
                taxesName = "运费税";
            }
            taxes.setCustomTax([
                ...
                {
                    id:"",
                    countryCode:taxes.taxInfo.country_code,
                    subTaxesConfigDTOList: [],
                    taxesDescSwitch: false,
    ...
    return (
        <>
            <DefaultButton text="添加自定义税率" onClick={() => {
                setOpen(true);
                form.resetFields();
                // 检查是否存在运费税
