...
function Textarea({setting,data,setSections,variables,rightRef}:{setting:settingType,data:any,setSections:(id:string,value:any)=>void,variables?:any,rightRef?:React.RefObject<HTMLDivElement>}) {
    ...
    const setVariable = (key:string)=>{
        if(oldKey.current && replacingElementRef.current && editableRef.current){
            ...
            // 超出长度限制时
            if (currentValue.length > (setting?.limit || 500) && editableRef.current) {
            ...
            // 超出长度限制时
            if (newValue.length > (setting?.limit || 500) && editableRef.current) {
                isLimit.current = true;
            }else{
                isLimit.current = false;
                // 更新内容
    ...
    const onChange = (e: React.FormEvent<HTMLDivElement>)  => {
        ...
        // 超出长度限制时
        if (displayValue.length > (setting?.limit || 500) && editableRef.current) {
            isLimit.current = true;
        }else{
            isLimit.current = false;
            // 更新内容
    ...
    return (
        <Scoped>
            ...
            <>
                ...
                <div className={`variable-limit ${isLimit.current ? "limit-exceeded" : ""}`}>{value.length}/{setting?.limit || 500}</div>
            </>
        </Scoped>
    )
}
