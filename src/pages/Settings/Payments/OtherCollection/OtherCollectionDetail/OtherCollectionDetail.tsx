...
function OtherCollectionDetail() {
    ...
    const fetch = (langId:string)=>{
        getAddonsConfigCreditCard(searchParams.get("id") || "",searchParams.get("addonsId") || "",langId).then(async res=>{
            if(res.data.addon_config){
                ...
                await payments.setOldOtherCollection(res.data.addon_config)
                Object.entries(JSON.parse(res.data.addon_config.config)).forEach(([key, value]) => {
                    payments.setOldOtherCollection({
                        ...payments.oldOtherCollection,
                        ["config["+key+"]"]:value
                    })
                    setRawData({
                        ...payments.oldOtherCollection,
    ...
    const submit = ()=>{
        setLoading(true)
        setAddonsConfigs(payments.oldOtherCollection).then(res=>{
            message.success("已更新")
        }).catch(()=>{
        }).finally(()=>{
            setLoading(false)
                ...
                <div className="mc-layout">
                    <div className="mc-header">
                        ...
                        <div className="mc-header-right">
                            {/* 语言 */}
                            <LangSelect lang={payments.oldOtherCollection.languages_id} setLang={setLang} />
                        </div>
                    </div>
                    <div className='mc-layout-main'>
                        <div className='mc-layout-content'>
                                ...
                                <div style={{margin:"20px 20px 0 20px"}}>
                                    <Form form={form} layout="vertical">
                                        {Object.entries(formItemList.config).map(([key, value])=>(
                                            <Form.Item key={key} label={<div className="font-w-600">{value.title}</div>}>
                                                {(()=>{
                                                    switch(value.type){
                                                        case "text":
                                                            return <Input value={(payments.oldOtherCollection as Record<string, any>)["config["+key+"]"] || ""} placeholder={value.title} onChange={(e)=>{
                                                                payments.setOldOtherCollection({
                                                                    ...payments.oldOtherCollection,
                                                        ...
                                                        case "select":
                                                            ...
                                                            return <Select value={(payments.oldOtherCollection as Record<string, any>)["config["+key+"]"] || null} placeholder={value.title} options={options} onChange={(value)=>{
                                                                payments.setOldOtherCollection({
                                                                    ...payments.oldOtherCollection,
                                                                    ["config["+key+"]"]:value
                                                                })
                                                        ...
                                                        case "radio":
                                                            return <Radio.Group
                                                                value={(payments.oldOtherCollection as Record<string, any>)["config["+key+"]"] || "0"}
                                        ...
                                        <Form.Item label={<div className="font-w-600">自定义标题</div>}>
                                            <Input value={payments.oldOtherCollection.title} onChange={(e)=>{
                                                payments.setOldOtherCollection({
                                                    ...payments.oldOtherCollection,
                                        ...
                                        <Form.Item label={<div className="font-w-600">自定义描述</div>}>
                                            <TextArea showCount maxLength={500} value={payments.oldOtherCollection.description} rows={5} onChange={(e)=>{
                                                payments.setOldOtherCollection({
                                                    ...payments.oldOtherCollection,
                                        ...
                                        <Form.Item label={<div className="font-w-600">备注</div>}>
                                            <TextArea showCount maxLength={200} value={payments.oldOtherCollection.remark} rows={2} onChange={(e)=>{
                                                payments.setOldOtherCollection({
                                                    ...payments.oldOtherCollection,
                                        ...
                                        <Form.Item label={<div className="font-w-600">排序</div>}>
                                            <Input value={payments.oldOtherCollection.sort} onChange={(e)=>{
                                                payments.setOldOtherCollection({
                                                    ...payments.oldOtherCollection,
                                                    sort:e.target.value
                                                })
                            ...
                            <Flex justify="space-between">
                                {payments.oldOtherCollection.status == 1?<DeleteModal tElement={
