...
function OtherCollectionAdd() {
    ...
    const submit = ()=>{
        setLoading(true)
        setAddonsConfigs(payments.newOtherCollection).then(res=>{
            message.success("添加成功")
            payments.clearNewOtherCollection()
    ...
    useEffect(()=>{
        getAddonsConfigCreditCard("",searchParams.get("addonsId") || "" ,searchParams.get("lang") || "").then(async res=>{
            ...
            payments.setNewOtherCollection({
                ...payments.newOtherCollection,
                                ...
                                <div style={{margin:"20px 20px 0 20px"}}>
                                    <Form form={form} layout="vertical">
                                        {Object.entries(formItemList.config).map(([key, value])=>(
                                            <Form.Item key={key} name={key} label={<div className="font-w-600">{value.title}</div>}>
                                                {(()=>{
                                                    switch(value.type){
                                                        case "text":
                                                            return <Input defaultValue={(payments.newOtherCollection as Record<string, any>)["config["+key+"]"] || ""} placeholder={value.title} onChange={(e)=>{
                                                                payments.setNewOtherCollection({
                                                                    ...payments.newOtherCollection,
                                                        ...
                                                        case "select":
                                                            ...
                                                            return <Select defaultValue={(payments.newOtherCollection as Record<string, any>)["config["+key+"]"] || ""} placeholder={value.title} options={options} onChange={(value)=>{
                                                                payments.setNewOtherCollection({
                                                                    ...payments.newOtherCollection,
                                                                    ["config["+key+"]"]:value
                                                                })
                                        ...
                                        <Form.Item label={<div className="font-w-600">排序</div>}>
                                            <Input defaultValue={payments.newOtherCollection.sort} onChange={(e)=>{
                                                payments.setNewOtherCollection({
                                                    ...payments.newOtherCollection,
                                                    sort:e.target.value
                                                })
