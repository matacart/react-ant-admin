...
function ThirdCreditCollectionDetail() {
    ...
    const fetch = (langId:string)=>{
        getAddonsConfigCreditCard(searchParams.get("id") || "",searchParams.get("addonsId") || "",langId).then(async res=>{
            if(res.data.addon_config){
                ...
                await payments.setOldThirdCreditCollection(res.data.addon_config)
                Object.entries(JSON.parse(res.data.addon_config.config)).forEach(([key, value]) => {
                    payments.setOldThirdCreditCollection({
                        ...payments.oldThirdCreditCollection,
                        ["config["+key+"]"]:value
                    })
                    setRawData({
                        ...payments.oldThirdCreditCollection,
                ...
                await payments.clearNewOtherCollection()
                history.push("/settings/payments/thirdCreditCard/add?addonsId="+searchParams.get("addonsId")+"&lang="+langId)
            }
        }).catch((err)=>{
            console.log(err)
        }).finally(()=>{
    ...
    const submit = ()=>{
        setLoading(true)
        setAddonsConfigs(payments.oldThirdCreditCollection).then(res=>{
            message.success("已更新")
        }).catch(()=>{
        }).finally(()=>{
            setLoading(false)
                ...
                <div className="mc-layout">
                    <div className="mc-header">
                        ...
                        <div className="mc-header-right">
                            {/* 语言 */}
                            <LangSelect lang={payments.oldThirdCreditCollection.languages_id} setLang={setLang} />
                        </div>
                    </div>
                    <div className='mc-layout-main'>
                        <div className='mc-layout-content'>
                                ...
                                <div style={{margin:"20px 20px 0 20px"}}>
                                    <Form form={form} layout="vertical">
                                        {Object.entries(formItemList.config).map(([key, value])=>(
                                            <Form.Item key={key} label={<div className="font-w-600">{value.title}</div>}>
                                                {(()=>{
                                                    switch(value.type){
                                                        case "text":
                                                            return <Input value={(payments.oldThirdCreditCollection as Record<string, any>)["config["+key+"]"] || ""} placeholder={value.title} onChange={(e)=>{
                                                                payments.setOldThirdCreditCollection({
                                                                    ...payments.oldThirdCreditCollection,
                                                        ...
                                                        case "select":
                                                            ...
                                                            return <Select value={(payments.oldThirdCreditCollection as Record<string, any>)["config["+key+"]"] || null} placeholder={value.title} options={options} onChange={(value)=>{
                                                                payments.setOldThirdCreditCollection({
                                                                    ...payments.oldThirdCreditCollection,
                                                                    ["config["+key+"]"]:value
                                                                })
                                                        ...
                                                        case "radio":
                                                            return <Radio.Group
                                                                value={(payments.oldThirdCreditCollection as Record<string, any>)["config["+key+"]"] || "0"}
                                        ...
                                        <Form.Item label={<div className="font-w-600">自定义标题</div>}>
                                            <Input value={payments.oldThirdCreditCollection.title} onChange={(e)=>{
                                                payments.setOldThirdCreditCollection({
                                                    ...payments.oldThirdCreditCollection,
                                        ...
                                        <Form.Item label={<div className="font-w-600">自定义描述</div>}>
                                            <TextArea showCount maxLength={500} value={payments.oldThirdCreditCollection.description} rows={5} onChange={(e)=>{
                                                payments.setOldThirdCreditCollection({
                                                    ...payments.oldThirdCreditCollection,
                                        ...
                                        <Form.Item label={<div className="font-w-600">备注</div>}>
                                            <TextArea showCount maxLength={200} value={payments.oldThirdCreditCollection.remark} rows={2} onChange={(e)=>{
                                                payments.setOldThirdCreditCollection({
                                                    ...payments.oldThirdCreditCollection,
                                        ...
                                        <Form.Item label={<div className="font-w-600">排序</div>}>
                                            <Input value={payments.oldThirdCreditCollection.sort} onChange={(e)=>{
                                                payments.setOldThirdCreditCollection({
                                                    ...payments.oldThirdCreditCollection,
                                                    sort:e.target.value
                                                })
                            ...
                            <Flex justify="space-between">
                                {payments.oldThirdCreditCollection.status == 1?<DeleteModal tElement={
...
export default observer(ThirdCreditCollectionDetail)
