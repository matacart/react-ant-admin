...
import manual from "@/store/settings/payments/manual";
...
function AddManualCollection() {
    ...
    const submit = ()=>{
        form.validateFields().then(()=>{
            setLoading(true)
            setAddonsConfig({...form.getFieldsValue(),config}).then(async res=>{
                await sleep(1000)
            ...
            <div className='mc-layout-wrap'>
                <div className="mc-layout">
                    <div className="mc-header">
                        ...
                        <Flex>
                            <LangSelect lang={manual.languagesId} setLang={(value:string)=>manual.setLanguagesId(value)} />
                        </Flex>
                    </div>
                    <div className='mc-layout-main'>
                        <div className='mc-layout-content'>
                            <Card classNames={{body:"card"}}>
                                <div style={{margin:"20px 20px 0 20px"}}>
                                    <Form form={form} layout="vertical">
                                        <Form.Item name="title" label={<div className="font-w-600">付款方式名称</div>} required={false} rules={[{ required: true, message: '请输入付款方式名称' }]}>
                                            <DefaultInput placeholder="输入付款方式名称" />
                                        </Form.Item>
                            ...
                            <Flex justify="flex-end">
                                <PrimaryButton loading={loading} onClick={submit} text="添加 手动收款方式" />
                            </Flex>
                        </div>
                    </div>
                </div>
