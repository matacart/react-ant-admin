...
function RemainingProduct({groupIndex}:{groupIndex:number}) {

    const shippedInfo  = orderRefund.remainingProductGroup[groupIndex]
            ...
            title={
                <Flex style={{ fontSize: "16px", color: "#474F5E"}} align="center" justify="space-between" >
                    <Flex align="center" justify="space-between" gap={10} style={{width:"100%"}}>
                        <div className="font-w-500">{"未发货"}（{shippedInfo.shipment.remaining_quantity_count}）</div>
                        ...
                        <Col span={5}>
                            ...
                            <div style={{marginTop:"12px"}}>
                                <NumberInput style={{width:"100px"}} value={item.num} min={0} max={item.remaining_quantity} onChange={(value:number)=>{
                                    const newRemainingProductGroup = toJS(orderRefund.remainingProductGroup)
                                    newRemainingProductGroup[groupIndex].product.forEach((product:any,index:number)=>{
                                    if(product.id===item.id){
                                        product.num=value??0
                                    }
                                    })
                                    orderRefund.setRemainingProductGroup(newRemainingProductGroup)
                                }} />
                            </div>
                        </Col>
                        <Col span={5}>
...
export default observer(RemainingProduct);