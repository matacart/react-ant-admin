...
import { getOrderDetail, getReturnReasonList } from '@/services/y2/apiStore';
import order, { ReturnReasonType } from '@/store/order/order';
...
function Index() {
  ...
  const controlsItems: MenuProps['items'] = [
    {
      label: <a onClick={()=>history.push(`/orders/productsEdit/${orderId}/${languagesId}`)}>编辑商品</a>,
      key: '1',
    },
    {
    ...
    },
    {
      ...
      key: '3',
      style: (order.ordersPackageList?.length > 0 || order.orderInfo?.bizOrderStatus == 400) ? { display: 'none' } : {  },
    },
    {
  ...
  useEffect(() => {
    getOrderDetail({
      ...
      order.setAfterSaleOrderList(res.data?.afterSaleOrderList || []);
              ...
              <Spin spinning={loading} indicator={<LoadingOutlined spin />} size="large">
                ...
                <Flex gap={20}>
                  <Flex className='mc-layout-content' vertical gap={20}>
                    ...
                    {order.afterSaleOrderList.map((item,index:number)=>{
                      return item.canCancelRefund?<OrderReturning key={index} groupIndex={index} returnReasonList={returnReasonList} />:<OrderReturned key={index} groupIndex={index} />
                    })}
                    {/* 取消包裹 */}
                    {/* deletedItemList */}
                    {/* 未付款/已付款 */}
