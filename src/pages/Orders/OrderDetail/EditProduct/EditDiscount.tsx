        ...
        form.validateFields().then((values)=>{
            ...
            newRemainingProductGroup[0].product.forEach((product:any,index:number)=>{
                if(item.id == "" ? product.vid == item.vid : product.id===item.id){
                    product.final_price=finalAmount
                    product.product_discount_amount=values.discount_amount
                    product.product_discount_type = discountType
                    product.product_discount_description=values.discount_explanation
                }
            })
