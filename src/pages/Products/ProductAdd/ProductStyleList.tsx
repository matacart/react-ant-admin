...
// newStore.setVariants([...oldData,...filteredArray])
...
function ProductStyleList(props:any){
  ...
  useEffect(() => {
    const generateStyles = async (sku:any) => {
      ...
      // setStyles(newStyles)
      // newStore.variants
      // setStyles([...newStyles,...newStore.variants])
      let temp = [...newStyles]
      console.log('newStore.variants',newStore.variants)
  ...
  useEffect(()=>{
    ...
    setCopyStyles(newStore.variants)
  },[])
  ...
  const handleSalePriceChange = (index: number,item:any,newValue: string) => {
    ...
    newStore.setVariants(newCopyStyles)
    // const updatedStyles = styles.map((style) => {
    //   if (style.keyId === id) {
    //     // 如果输入为空字符串，则设置为 0
    ...
    // });
    // setStyles(updatedStyles);
    // newStore.setVariants(updatedStyles);
  };
  // 添加处理发货状态变化的函数
...
const handleRemove = (item:any,index:number) => {
  ...
  if(item.id){
    ...
    newStore.setVariants(newCopyStyles.filter(res=>item.option_values_names !== res.option_values_names))
