...
function SEOEdit({seo,setSEO,type}:{seo:any,setSEO?:(title:string,description:string,keyword:string,url:string)=>void,type:string}){
    ...
    useEffect(()=>{
        ...
        if(seo.url == ""){
            ...
            // 解析
            // const urlObject = new URL(seo.url)
            // 解析数据为空
            if(seo.url.slice(1,seo.url.lastIndexOf(type)) == ""){
                setUrl(seo.title.trim().replace(new RegExp(" ","gm"),"-"))
            }else{
                setUrl(seo.url.slice(1,seo.url.lastIndexOf(type)))
            }
        }
    },[seo.title,seo.url])
        ...
        <div>
            ...
            <Drawer width={600} title='搜索引擎优化' open={open} onClose={()=>{
                ...
                if(seo.url == ""){
                    ...
                    // setUrl(seo.url.slice(1,seo.url.lastIndexOf("-p")))
                }
            }}>
                <Scoped>
                    <div className="contentCard">
                        ...
                        <Form
                            ...
                            <Form.Item label="搜索引擎关键词" tooltip="关键词可以提高搜索结果排名，建议1-2个关键词即可，堆砌关键词可能会降低排名！">
                                <Input placeholder="输入关键词后，按enter键完成输入" value={keyword} defaultValue={seo.meta_keywords} onChange={(e)=>{
