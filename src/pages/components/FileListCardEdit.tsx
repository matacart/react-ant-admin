...
import { Button, Col, Image, GetProp, Input, Row, Select, Upload, UploadFile, UploadProps, Tooltip, Modal, Spin, PaginationProps, Pagination, message, InputRef } from "antd"
...
function FileListCard({groupId}:{groupId:string}) {
    ...
    // 选择图片 --- 增减
    const handleSelectImg = (imgObj,index:number)=>{
        let newFileListStatus = {...fileListStatus}
        let newIndex = oldStore.fileUpload.selectFileIndex
        let newSelectFileList = [...oldStore.fileUpload.selectFileList]
        if(newFileListStatus[imgObj.id]){
            ...
            oldStore.fileUpload.setSelectFileIndex(newIndex)
            newSelectFileList = newSelectFileList.filter(item=>item.id !== imgObj.id)
        }else{
            // 添加
            newFileListStatus[imgObj.id] = true
            oldStore.fileUpload.selectFileIndex[imgObj.id] = Object.keys(oldStore.fileUpload.selectFileIndex).length+1
            newSelectFileList.push(imgObj)
        }
        setFileListStatus(newFileListStatus)
        oldStore.fileUpload.setSelectFileList(newSelectFileList)
    }
                ...
                <div className="cardContent">
                    <Row gutter={[8,8]}>
                        ...
                        {fileImgList.map((item:any,index:number)=>(
                            <Col className="gutter-row" span={6}>
                                <div className="item_card_box" onClick={()=>{handleSelectImg(item,index)}}>
                                    {fileListStatus[item.id] && <div className="item_card_box_status">{oldStore.fileUpload.selectFileIndex[item.id]}</div>}
