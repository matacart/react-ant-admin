...
import { jsonTemplate } from "../Header/Header";
...
export default function RenameTemplateModal({
    ...
    useEffect(()=>{
        const newOptions = templateList.map(item=>{
            ...
            return {
                ...
                label: isDefaultTemplate ? intl.formatMessage({id:'theme.design.header.navigation.default'}):item.pageName,
            }
        })
    ...
    return (
        ...
        <Scoped onClick={(e)=>e.stopPropagation()}>
            <Tooltip title={intl.formatMessage({id:'theme.design.header.navigation.tipRename'})}>
                <EditIcon
            ...
            <Modal width="480px" title={intl.formatMessage({id:'theme.design.header.navigation.renameTemplate'})} centered open={open} 
            ...
            >
                <div style={{marginTop:"24px"}}>
                    <WarningTagText content={<span className="color-242833 font-14">{intl.formatMessage({id:'theme.design.header.renameTemplateModalWarningText'})}</span>} />
                </div>
                <MyForm 
                    form={form} 
                    layout="vertical"
                ...
                >
                    <Form.Item label={intl.formatMessage({id:'theme.design.header.modalTemplateName'})} name="name" required={false} rules={[
                        { required: true },
                        { 
                            validator: (_, value) => {
                        ...
                        <DefaultInput maxLength={25} showCount placeholder={intl.formatMessage({id:'theme.design.header.modalTemplateNamePlaceholder'})} /> 
                    </Form.Item>
                </MyForm>
            </Modal>
