...
function ComponentRight({templateUpdatePromiseRef}:{templateUpdatePromiseRef: React.MutableRefObject<Promise<any> | null>}){
    ...
    const debouncedSetData = useCallback(
        debounce(async (item: any, value: any) => {
            // 创建一个新的 promise 来跟踪当前的 templateUpdate 操作
            const currentUpdatePromise = (async () => {
            ...
            // 将当前操作的 promise 存储起来
            templateUpdatePromiseRef.current = currentUpdatePromise;
            // 等待当前操作完成
                                ...
                                {componentConfig.components?.map((item:any,index:number)=>{
                                    switch (item.type) {
                                        ...
                                        case "video_text":{
                                            ...
                                            const isYoutubeUrl = urlPattern.test(data?.value || "");
                                            return(
                                                <Form.Item key={componentConfig.id+'-'+index} label={<div>
                                                    <div>{intl.formatMessage({id: item.label})}</div>
                                                </div>}>
                                                    <ItemVideo item={item} data={isYoutubeUrl?{}:data} setData={debouncedSetData} />
                                                    {(!data?.value || isYoutubeUrl) && <>
                                                        {item.info && <div className="font-12 color-7A8499" style={{marginBottom:"4px",marginTop:"20px"}}>{intl.formatMessage({id: item.info})}</div>}
