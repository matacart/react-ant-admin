            ...
            switch(editor.component.type){
                case 'section':
                    ...
                    if(editor.component?.itemId){
                        ...
                        if(Array.isArray(section.config.settingsData.settings)){
                            // 转对象
                            setComponentsData({})
                        }else{
                    ...
                    break;
                case 'template':
                    const template = editor.templateData.find(res => res.type == "TEMPLATE").sections[editor.component?.id]
                    if(editor.component?.itemId){
                        ...
                        <Form layout="vertical" className="form color-474F5E">
                            {components?.map((item:any,index:number)=>{
                                switch (item.type) {
                                    case "group_header":
                                        return (
                                            <div key={index} style={{marginBottom:"16px",fontWeight:500}}>
                                        ...
                                        return (
                                            <Form.Item key={index} label={false}>
                                                <Flex gap={8} align="center">
                                                    ...
                                                    <div>
                                                        <div>{intl.formatMessage({id: item.label})}</div>
                                                        <div style={{height:"18px"}} className="font-12 color-7A8499">{currentColor}</div>
                                                    </div>
                                                </Flex>
                                            </Form.Item>
                                        )
