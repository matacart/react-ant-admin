...
function LayoutModal({filePath,treeData,setTreeData}:LayoutModalType){
        ...
        form.validateFields().then((values:any)=>{
            ...
            addTemplateFile({
                ...
                if(res.code == 0){
                    ...
                    // 来源文件树
                    const {newTreeData,level} = insertFileInTree(treeData,(fileSource+"."+values.name),fileType,(fileSource+'.'+sourcePath),"layout");
    ...
    useEffect(()=>{
        const newOptions = treeData.filter(treeItem=>treeItem.title == "layout")[0].children.map((childItem:any)=>{
            if(childItem.type){
                return
            }
            return{
    ...
    return (
        <Scoped>
            <Flex className="add color-356DFF" onClick={()=>setOpen(true)}>新建layout</Flex>
            {/*  */}
            <Modal
                open={open}
                width={"480px"}
            ...
            >
                <Form form={form} layout="vertical" style={{margin:"24px 0"}}>
                    ...
                    >
                        <MySelect style={{height:"36px"}} placeholder="来源" options={options} />
                    </Form.Item>
                    <Form.Item
                        label="将其命名为"
                        name="name"
